...
describe("Use case: Registration Flow (all successful)", () => {
  ...
  test("Create user account", async () => {
    const createUserResponde = await fetch(`${webserver.origin}/api/v1/users`, {
      method: "POST",
      headers: {
  ...
  test("Receive activation email", async () => {
    ...
    expect(lastEmail.sender).toBe("<contato@bytetab.com.br>");
    expect(lastEmail.recipients[0]).toBe("<registration.flow@curso.dev>");
    expect(lastEmail.subject).toBe("Ative seu cadastro no ByteTab!");
    ...
    expect(activationTokenObject.user_id).toBe(createUserRespondeBody.id);
  ...
  test("Login", async () => {
    ...
    expect(createSessionResponseBody.user_id).toBe(createUserRespondeBody.id);
  });
